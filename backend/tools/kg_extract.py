...
async def run(job: Job, ctx: ToolContext, update_progress) -> Dict[str, Any]:
    ...
    kb_doc_id = str(payload.get("kb_doc_id") or "").strip()
    if not graph_id or (not text and not kb_doc_id):
        return {"ok": False, "error": "graph_id + (text or kb_doc_id) required"}
    ...
    try:
        if kb_doc_id:
            kb_chunks = ctx.kb_retriever.kb.list_chunks(doc_ids=[kb_doc_id], limit=100000, include_text=True)
            if not kb_chunks:
                return {"ok": False, "error": f"KB doc not found or empty: {kb_doc_id}"}

            total_units = max(1, len(kb_chunks))
            for i, kb_chunk in enumerate(kb_chunks, start=1):
                ctx.check_job_cancelled(job.id)
                update_progress(0.10 + 0.80 * min(1.0, i / total_units))
                ...
                async for c in iter_extract_kg_chunks(model_spec=model_spec, text=chunk_text_full, ontology=ontology):
                    ...
                    if entities:
                        ...
                        store.link_mentions(chunk_id=kg_chunk_id, entity_uuids=entity_uuids, graph_id=graph_id)
            ...
            idx = 0
            step = max(1, 1200 - 120)
            est_chunks = max(1, int(math.ceil(len(text) / step)))
            async for c in iter_extract_kg_chunks(model_spec=model_spec, text=text, ontology=ontology):
                ctx.check_job_cancelled(job.id)
                idx += 1
                extracted_chunks += 1
                update_progress(0.10 + 0.80 * min(1.0, idx / est_chunks))
